...
defineProps<{ modelValue: boolean; naslov: string }>()
  ...
  <Teleport to="body">
    <div v-if="modelValue" class="overlay" @click.self="emit('update:modelValue', false)">
      <div class="modal" role="dialog" :aria-label="naslov">
        ...
        <div class="modal-sadrzaj"><slot /></div>
...
.zatvori {
  font-size: 1rem; color: var(--boja-tekst-mute);
  background: none; border: none; cursor: pointer; padding: 0.25rem;
}
.zatvori:hover { color: var(--boja-akcent); }
.modal-sadrzaj { padding: 1.5rem; display: flex; flex-direction: column; gap: 1rem; }
.modal-akcije {
  display: flex; gap: 0.75rem; justify-content: flex-end;
  padding: 1rem 1.5rem; border-top: 1px solid var(--boja-rub);
}
</style>
